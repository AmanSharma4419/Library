...
var ev = [];
class DraftComposMail extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isDirty: false,
  ...
  getdata = () => {
    const getdataid = sessionStorage.getItem("drcomposeid");
    axios
      .get(
        baseurl +
        "/get_inboxemail/" +
      ...
      .then((response) => {
        console.log("outboxre", response.data.draft_mail, getdataid);
        // countdata = response.data.mail.length || '0';
        for (var i = 0; i < response.data.draft_mail.length; i++) {
          if (response.data.draft_mail[i].id == getdataid) {
            const cc = response.data.draft_mail[i].mail_cc
              ? response.data.draft_mail[i].mail_cc
              : "";
            const bcc = response.data.draft_mail[i].mail_bcc
              ? response.data.draft_mail[i].mail_bcc
              : "";
            const mailmessage =
              "<p></p><p><b>From:</b> " +
              response.data.draft_mail[i].mail_from +
              "</p><p><b>Sent:</b> " +
              new Date(response.data.draft_mail[i].updated_date) +
              "</p><p><b>To:</b> " +
              response.data.draft_mail[i].mail_to +
              "</p><p><b>CC:</b> " +
              cc +
              "</p><p><b>BCC:</b> " +
              bcc +
              "</p><p><b>Subject:</b> " +
              response.data.draft_mail[i].subject +
              "</p><p>" +
              response.data.draft_mail[i].message +
              "</p>";

            this.setState({
              mail_cc: response.data.draft_mail[i].mail_cc,
              mail_bcc: response.data.draft_mail[i].mail_bcc,
              mail_to: response.data.draft_mail[i].mail_to,
              subject: response.data.draft_mail[i].subject,
              message: response.data.draft_mail[i].message,
              editorState: EditorState.createWithContent(
                ContentState.createFromBlockArray(
                  convertFromHTML(response.data.draft_mail[i].message)
                )
              ),
              selectedFile: response.data.draft_mail[i].files,
            });
            ev = response.data.draft_mail[i].files;
          }
        }
      });
  };
  ...
  OnDraftSent = (e) => {
    ...
    this.state.formData.append("id", sessionStorage.getItem("drcomposeid"));
                          ...
                          <CardBody className="pl-4">
                            ...
                            <InputGroup style={inputgroupbox}>
                              ...
                              <Input
                                ...
                                onChange={(e) => {
                                  this.setState({ mail_cc: e.target.value });
                                }}
                              />
                            </InputGroup>
                            {this.state.isTogglebcc == "true" ? (
                              <InputGroup style={inputgroupbox}>
                                ...
                                <Input
                                  ...
                                  onChange={(e) => {
                                    this.setState({ mail_bcc: e.target.value });
                                  }}
                                />
                              </InputGroup>
                            ) : null}
...
export default DraftComposMail;
