...
function submitForm(contentType, data, setResponse, path) {
  axios({
  url: baseurl+`/storestudentbackgroundinfo_app`,
  method: 'POST',
  data: data,
  headers: {
...
class AppBackgroundinfo extends React.Component 
{
  ...
  constructor(props) {
	...
	this.getQuestiondata3 = this.getQuestiondata3.bind(this);
	this.setQuestiondata3 = this.setQuestiondata3.bind(this);
    this.state = {
      collapse: true,
...
setQuestiondata3(index) {
  ...
  getBackgroundinfodata()
  {
      ...
      axios.get(baseurl+'/get_backgroundinfo_app/'+localStorage.getItem('studentid')).then(response => {
