...
import template from "./EduAddMore.jsx";
...
class EduAddmore extends React.Component {
  ...
  constructor(props) {
    ...
    this.clearEduaddmore = this.clearEduaddmore.bind(this);
...
console.log("jay" + response)
  ...
  componentDidUpdate(prevProps) {
    if(this.props.clearEduaddmore !== prevProps.clearEduaddmore) {
      if(this.props.clearEduaddmore) this.clearEduaddmore()      
    }
  ...
  getQuessummary() {
	   var param = localStorage.getItem('universityid');
	var stdid = localStorage.getItem('studentid');
	axios.get(baseurl + '/get_answer/'+ param +'/'+ stdid +'/education').then(response =>{
...
var str2 = arrayString.replace(/",/g,"~"); 
 ...
 this.state.formData.append("question_id",this.state.questionId);
	...
	this.state.formData.append("type", 'education');
