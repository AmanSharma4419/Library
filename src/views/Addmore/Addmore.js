...
class Addmorequestion extends Component {
  ...
  constructor(props) {
    ...
    this.clearAddmore = this.clearAddmore.bind(this);
...
//alert(localStorage.getItem('studentid'));
  ...
  /*  axios.get(baseurl + '/get_questionpattern/' + param).then(response => {
console.log("jay" + response)
	...
	var param = localStorage.getItem('universityid');//alert(param);
	var stdid = localStorage.getItem('studentid');
	axios.get(baseurl + '/get_answer/'+ param +'/'+ stdid +'/general').then(response => {
console.log("jay" + response)
      this.setState({
  ...
  getQuessummary() {
   ...
   // axios.get(baseurl + '/get_questionpattern/' + localStorage.getItem('universityid')).then(response => {
	  // var param = 123;
 //axios.get(baseurl + '/get_questionpattern/' + 123).then(response => {
	 //var stdid = 33;
	 var param = localStorage.getItem('universityid');//alert(param);
	var stdid = localStorage.getItem('studentid');
	axios.get(baseurl + '/get_answer/'+ param +'/'+ stdid +'/general').then(response =>{
  ...
  onSelectQuestion = (index) => {
    ...
    // alert(JSON.stringify(['not modified',this.state.question[index]]))
  ...
  handleQuestionName = (event, index) => {
		...
		// keyvalu = event.target.value.replace(/",/g,"~")
		//var str2 = arrayString.replace(/",/g,"~"); 
		
		//alert(this.state.handleanswers);
 ...
 /* handleQuestionName = (event, index) => {
    const handleQuestionclone = this.state.handlequestion
    handleQuestionclone[index] = event.target.value
    this.setState({ handlequestion: handleQuestionclone });
  };*/
...
var str2 = arrayString.replace(/",/g,"~"); 
//alert(str2);
var singleoption = this.state.typeoption1.join();
//alert(singleoption);
    // alert(JSON.stringify(handlequestionArr))
    ...
    this.state.selectedOption.map((option, index) => {
      ...
      if (option == 2) {
        ...
        if (this.state.addOnChange[index]) {
          let addOn;
          // let addOnLength = addOn.length;
          addOn = this.state.addmoreinput[index].join("@")
          firstElement = firstElement + "@" + addOn;
          // if(addOnLength > 0) firstElement = firstElement + "@" + addOn;
        }
        merged.push(firstElement)
      }
      if (option == 0) {
 ...
 //   this.state.formData.append("options", merged);
	...
	this.state.formData.append("type", 'general');
