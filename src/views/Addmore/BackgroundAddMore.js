...
import template from "./BackgroundAddMore.jsx";
...
class BackgroundAddMore extends React.Component {
  ...
  constructor(props) {
    ...
    this.clearBackgroundAddmore = this.clearBackgroundAddmore.bind(this);
 ...
 var param = localStorage.getItem('universityid');
	...
	axios.get(baseurl + '/get_answer/'+ param +'/'+ stdid +'/background').then(response => {
console.log("jay" + response)
  ...
  componentDidUpdate(prevProps) {
    if(this.props.clearBackgroundAddmore !== prevProps.clearBackgroundAddmore) {
      if(this.props.clearBackgroundAddmore) this.clearBackgroundAddmore()      
    }
...
var str2 = arrayString.replace(/",/g,"~"); 
 ...
 //   this.state.formData.append("options", merged);
	...
	this.state.formData.append("type", 'background');
...
export default BackgroundAddMore;
